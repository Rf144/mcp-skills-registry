...
- 修改或建立檔案
- **執行任何 Bash / 終端機指令**（包含 `mkdir`、`cp`、`npm`、`vercel` 等）
- **執行任何 git 指令**（包含 `git init`、`git add`、`git commit`、`git push`）
- 安裝或移除套件
- 呼叫外部 API 或服務（包含 MCP 工具）
- 刪除任何內容

**重要：不可以連續執行多個指令而不說明。** 每一個指令都要個別說明並等待確認，或在說明時一次列出「接下來要依序執行這幾個指令」再等確認。
...
說明完後等使用者說「好」、「繼續」、「開始吧」或類似確認語才執行。如果使用者說「不用每次問」或「直接做」，則在該對話中跳過這個行為。
...
### Step 5 — 確認 Repo 可見性
推完後，主動檢查今天推送的 repo 是否含有 API Key 或敏感資訊：
- **有 API Key / token 硬寫在程式碼裡** → 主動提醒使用者設為 Private
- **純前端但有第三方 API 串接** → 同上，建議設為 Private
- **無敏感資訊的純靜態頁或文件** → 可維持 Public

設為 Private 的方式：用 GitHub MCP 或提供設定連結讓使用者手動操作。

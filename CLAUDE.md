...
**功能說明：** 直接操作 Supabase 專案，涵蓋資料庫查詢、schema 管理、SQL 執行、Edge Functions、分支管理、費用查詢等。
...
*組織管理*
- `list_organizations` / `get_organization` — 列出或查看組織

*資料庫*
- `execute_sql` — 直接執行 SQL 語句
- `list_tables` / `apply_migration` — 管理資料表與 schema 遷移
- `list_migrations` — 查看遷移記錄
- `list_extensions` — 列出資料庫擴充套件
- `generate_typescript_types` — 根據 schema 產生 TypeScript 型別

*Edge Functions*
- `list_edge_functions` / `get_edge_function` / `deploy_edge_function` — 管理 Edge Functions

*分支管理*
- `create_branch` / `delete_branch` / `list_branches` — 建立、刪除、列出分支
- `merge_branch` / `rebase_branch` / `reset_branch` — 分支合併、rebase、重置

*監控與費用*
- `get_logs` — 查閱專案執行日誌
- `get_advisors` — 取得效能與安全建議
- `get_cost` / `confirm_cost` — 查詢與確認費用

*文件*
- `search_docs` — 搜尋 Supabase 官方文件

**使用場景：** 查詢或修改 Supabase 資料庫、管理 table schema、除錯 Edge Functions、管理分支環境、取得專案設定資訊。
...
- `create_record` / `update_records` / `delete_records` — 新增、更新、刪除記錄
...
#### antonio-workflow
- **觸發時機：** 說「開工」、「閱讀 antonio-workflow」、「套用工作流程」時啟用；說「收工」時執行完整收工流程
- **功能說明：** Antonio 的個人工作流程，包含三個核心行為：(1) 每次動作前用繁體中文說明並等待確認，(2) 說「收工」時自動更新 Obsidian 筆記、整理 md、建立日誌並推上 GitHub，(3) 優先使用已連接的 MCP 工具，不索取 token
- **skill 路徑：** `skills/antonio-workflow/SKILL.md`
